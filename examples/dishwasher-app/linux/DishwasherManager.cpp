...
namespace RefAlarmAttr   = chip::app::Clusters::DishwasherAlarm::Attributes;
...
DishwasherManager DishwasherManager::sDishwasherMgr;
...
constexpr const uint8_t kNamespaceDishwasher = 0x41;
...
int8_t DishwasherManager::ConvertToPrintableTemp(int16_t temperature)
{
...
void DishwasherManager::RefAndTempCtrlAttributeChangeHandler(EndpointId endpointId, AttributeId attributeId, uint8_t * value, uint16_t size)
{
    switch (attributeId)
    {
    ...
    default: {
        ChipLogError(AppServer, "Unhandled Dishwasher and Temperature attribute %ld", attributeId);
        return;
    }
    break;
    }
...
void DishwasherManager::TempCtrlAttributeChangeHandler(EndpointId endpointId, AttributeId attributeId, uint8_t * value, uint16_t size)
{
    switch (attributeId)
    {
...
void DishwasherManager::RefAlarmAttributeChangeHandler(EndpointId endpointId, AttributeId attributeId, uint8_t * value, uint16_t size)
{
    switch (attributeId)
    {
    case RefAlarmAttr::Mask::Id: {
        auto mask = static_cast<uint32_t>(*value);
        mState    = static_cast<chip::app::Clusters::DishwasherAlarm::AlarmBitmap>(mask);
    ...
    case RefAlarmAttr::State::Id: {
        ...
        mState     = static_cast<chip::app::Clusters::DishwasherAlarm::AlarmBitmap>(state);
    ...
    case RefAlarmAttr::Supported::Id: {
        ...
        mSupported     = static_cast<chip::app::Clusters::DishwasherAlarm::AlarmBitmap>(supported);
        RefAlarmAttr::Supported::Set(endpointId, mSupported);
    }
    break;

    default: {
        ChipLogError(AppServer, "Unhandled Dishwasher Alarm attribute %ld", attributeId);
        return;
    }
    break;
    }
